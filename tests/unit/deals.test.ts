...
beforeEach(() => {
  ...
  malabar = createRefiner({
    id: 'malabar', name: 'Malabar Refining', region: 'South_Asia', cash: 3_000_000, techTier: 3,
    processingCapacity: 9000, crudeStorageCapacity: 30_000,
  });
...
const terms = (over: Partial<DealTerms> = {}): DealTerms => ({
  ...
  qtyPerDay: 5000, termDays: 30, price: 60, avoidChokepoints: [], ...over,
});
...
describe('signing (spec G4.3)', () => {
  it('starts tomorrow and runs for the term', () => {
    expect(sign()).toMatchObject({ dealId: 'deal-000001', startTick: 11, endTick: 41, status: 'ACTIVE', qtyPerDay: 5000, price: 60 });
  });

  it('refuses terms the companies could not carry out', () => {
    expect(() => sign({ qtyPerDay: 2500 })).toThrow(/whole number of lots/);
    expect(() => sign({ qtyPerDay: 11_000 })).toThrow(/1000–10000/);
  ...
  it('keeps each side within DEAL_MAX_SHARE of its capacity', () => {
    const first = sign({ qtyPerDay: 5000 });
    // Qasr pumps 9,000 a day: 80% is 7,200, so another 3,000 on top of 5,000 is too much.
    expect(() => sign({ qtyPerDay: 3000 }, [first], 2)).toThrow(/more than 80%/);
    expect(sign({ qtyPerDay: 2000 }, [first], 2).qtyPerDay).toBe(2000);
  });
});
...
describe('daily delivery (spec §5 phase 5a)', () => {
  it('delivers nothing before its first day', () => {
    ...
    expect(qasr.storage).toBe(20_000);
  });

  it('loads the day’s volume: the buyer pays price and freight, the seller pays its tariff', () => {
    const deal = sign();
    expect(deliver([deal], 11)).toEqual([{ dealId: deal.dealId, delivered: 5000, shortfall: 0, held: 0 }]);
    expect(qasr.storage).toBe(15_000);
    // Middle_East → South_Asia through Hormuz: $1.00 freight. Middle_East export tariff $0.20.
    expect(malabar.cash).toBeCloseTo(3_000_000 - 5000 * 60 - 5000 * 1.0, 6);
    expect(qasr.cash).toBeCloseTo(2_000_000 + 5000 * 60 - 5000 * 0.2, 6);
    expect(malabar.inboundBarrels).toBe(5000);
    expect(cargo).toHaveLength(1);
    expect(cargo[0]).toMatchObject({ ownerId: malabar.agentId, qty: 5000, dealId: deal.dealId, status: 'MOVING', awaitingRoute: false });
  ...
  it('compensates the buyer for barrels the seller could not load', () => {
    ...
    const penalty = 0.15 * 60 * 3000;
    expect(qasr.cash).toBeCloseTo(2_000_000 + 2000 * 60 - 2000 * 0.2 - penalty, 6);
    expect(deal).toMatchObject({ deliveredBbl: 2000, shortfallBbl: 3000 });
  });
  ...
  it('delivers exactly qty_per_day a day, split between delivered and shortfall (invariant 10), and ends on time', () => {
    ...
    expect(deal.deliveredBbl + deal.shortfallBbl).toBe(30 * 5000);
  });

  it('with Hormuz closed, fills the Oman bypass then the Red Sea bypass, ahead of spot trade', () => {
    setChokepoint(graph, 'HORMUZ', 'CLOSED');
    const deal = sign({ qtyPerDay: 7000 });
    deliver([deal], 11);
    expect(cargo.map((c) => [c.route.edges[0], c.qty])).toEqual([['bypass_red_sea', 4000], ['bypass_oman', 3000]]);
  ...
  it('holds loaded cargo at the origin when no route has space, and sends it when space frees', () => {
    setChokepoint(graph, 'HORMUZ', 'CLOSED');
    for (const [edge, qty] of [['bypass_oman', 1500], ['bypass_red_sea', 3000]] as const) setReservation(graph, asEdgeId(edge), asAgentIdOf('rival'), qty, DEFAULT_CONFIG);
    ...
    expect(d).toMatchObject({ delivered: 5000, held: 5000 });
    expect(cargo[0]).toMatchObject({ status: 'HELD', awaitingRoute: true, qty: 5000 });
    expect(malabar.cash).toBeCloseTo(cash - 5000 * 60, 6);   // paid for, no freight yet
    ...
    expect(sum(cargo.filter((c) => !c.awaitingRoute))).toBe(4500);
    expect(sum(cargo.filter((c) => c.awaitingRoute))).toBe(5500);
...
describe('commitments, cancelling and splitting', () => {
  it('counts tomorrow’s deliveries for the producer rule (spec §6.1)', () => {
    ...
    expect(dealCommitments([deal], qasr.agentId, 11)).toBe(5000);
  ...
  it('charges the party that walks away CANCEL_RATE of the value still to come, paid to the other', () => {
    ...
    expect(fee).toBeCloseTo(0.1 * 60 * 5000 * 20, 6);
    expect(malabar.cash).toBeCloseTo(3_000_000 - fee, 6);
    expect(qasr.cash).toBeCloseTo(2_000_000 + fee, 6);
  ...
  it('splits a deal into two whole-lot halves with different routes (spec §4.4)', () => {
    const deal = sign({ qtyPerDay: 5000 });
    const [keep, moved] = splitDeal(deal, ['HORMUZ'], [2, 3], 15, DEFAULT_CONFIG);
    expect(deal.status).toBe('ENDED');
    expect([keep.qtyPerDay, moved.qtyPerDay]).toEqual([2000, 3000]);
    ...
    expect(() => splitDeal(sign({ qtyPerDay: 1000 }), [], [4, 5], 15, DEFAULT_CONFIG)).toThrow(/single lot/);
  });
});
