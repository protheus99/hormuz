...
import { regionName, regionsFor, type Difficulty, type GameSettings, type PlayType } from '../src/game';
...
interface Choice { type: PlayType; region: string; name: string; difficulty: Difficulty; length: number | null; second: string }

export function showNewGame(root: Element, hasSave: boolean, start: (s: GameSettings) => void, resume: () => void): void {
  const state: Choice = { type: 'PRODUCER', region: 'US_Permian', name: 'Lone Star Crude', difficulty: 'NORMAL', length: 365, second: '' };
