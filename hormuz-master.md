...
**Status:** In build — engine Phases 0–7, the game session (Phase 8) and decision cards (Phase 9) complete; the Phase 10 interface and Phase 11 events, news, campaign and difficulty are built, Phase 12's first balance pass is done, and the owner's first two rounds of playtest notes are answered (D45, D46); the open balance and process questions are in `QUESTIONS.md`. This is the single source of truth for the Hormuz game and its market engine (GEMS, the Global Energy Market Simulator). It supersedes all earlier GEMS specifications and prototypes.
