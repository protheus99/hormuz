...
- **The news strip** under the top bar, one piece of furniture carrying three tiers, so that what
  changes the business is not filed behind a tab with what does not. **Calm:** it crawls — prices,
  rivals, the world; a Hold button stops it, and it does not move at all for a reader who has asked
  for less motion. **Yours:** an alert at `MEDIUM` or above is pinned *still* at the front until it
  is read, while the rest carries on behind it. **Breaking:** whatever stopped the clock halts the
  crawl, grows the band and turns it, and offers Continue at the speed the clock was running. The
  motion stopping is the signal. A new card is left to the inbox, which already announces it twice.
...
A chokepoint's `status` is `OPEN`, `TENSION`, `DELAYED` or `CLOSED`. `TENSION` adds its `freight_surcharge` (war-risk insurance) to the edge with no delay. `DELAYED` adds `delay_ticks` and keeps any surcharge. `CLOSED` removes the edge from routing; cargo already at the entry waypoint holds position until it reopens. **Throughput falls in steps with the status (D35):** each chokepoint lane carries a daily `throughput` when `OPEN` (Malacca 40,000 bbl; Hormuz 30,000; Suez and Bab el-Mandeb 20,000; Bosphorus and the Danish Straits 15,000; Panama 10,000 — about 3–4 times a typical day, so an open strait binds only on the busiest days), scaled by `CHOKEPOINT_THROUGHPUT`: 100% `OPEN`, 75% `TENSION`, 50% `DELAYED`, 0% `CLOSED`. Once a narrowed strait is full, cargo is redirected exactly as when a pipeline fills — to a bypass pipeline or another sea route. All ten chokepoints use these statuses; how often each one changes, and how far, is set by its event profile (G7.1) or, for the three the sea closes, by its storm season (below). **Closing any single chokepoint leaves every region at least one route to market** — only a pipeline's capacity can limit it — and a property test enforces this (§14.6).

**Weather conditions (2026-09-26).** Seven chokepoints are closed by governments. Three are closed by the sea, and are deliberately named apart from the *economic climate* (§12A.8 B): that is what the market is doing, this is what the water is doing, and the two never speak to each other.

| Passage | Lane | Season | Odds in season | Worst it does |
|---|---|---|---|---|
| **Gulf of Mexico** | Yucatán Channel (`caribbean_gulf`) | days 152–334, June to November | 1 in 40 | `DELAYED` only |
| **Cape of Good Hope** | `cape_s_atlantic` | days 121–273, the southern winter | 1 in 55 | `CLOSED` |
| **Cape Horn** | `cape_horn` (new) | days 121–243 | 1 in 30 | `CLOSED` |

A storm runs `WEATHER.DAYS` (2–6), adds `WEATHER.SURCHARGE` ($0.35/bbl) and `WEATHER.DELAY` (2 ticks), and a `CLOSE_SHARE` (one in five) of them close the water outright where the passage allows it. Out of season the odds fall to `OFF_SEASON` (15%) of the figure above. Storms start only on water that is otherwise open and lift only what the weather is still holding — `ChokepointState.byWeather` records who shut a passage, because a storm and a government both look like `CLOSED` and the weather must never lift a government's closure. Measured over five years × two seeds: the Gulf storms about six times a year for 96–117 congested days, the capes 13–27 times for 37–87.

**Two lanes were added with them, both to keep §14.6 true.** The **Straits of Florida** (`florida_straits`, W_GULF_MEXICO ↔ W_N_ATLANTIC, 11 ticks, $1.50) give the Gulf the second exit it has in life: with only the Yucatán Channel, putting a chokepoint there stranded Houston and Campeche the moment it closed, and the invariant test caught it. It is priced a little above the Caribbean route so nothing reroutes in fair weather. **Cape Horn** (`cape_horn`, W_S_ATLANTIC ↔ W_N_PACIFIC, 32 ticks, $3.80) is the western way between the oceans. It is *not* the only alternative to Panama — east round the Cape of Good Hope has always been there, and an earlier note claiming otherwise was wrong. Its price was set by measurement: at $2.50 it beat the canal on generalised cost ($5.70 against $6.50) and took half the traffic between the oceans, which is not what a bypass is for. At $3.80 **no region pair, of 462, takes it as their cheapest route**, and what crosses it is what the canal could not take that day. A test holds that property.
...
Figures below are after the ×20 rescale of 2026-09-26 (D64); every ratio is exactly as it was.

| | bbl/tick |
|---|---|
| Production capacity | 1,770,000 |
| Refining capacity | 1,980,000 |
| Consumed at `BASE_UTILIZATION` (0.85) | 1,683,000 |
| **Surplus** | **87,000 (5.2%)** |

Grade access also clears: Heavy Sour, which only Tier 3 plants accept, has 500,000 bbl/tick of production against 880,000 of Tier 3 capacity, and Tier 1 capacity is 220,000 against 560,000 of Light Sweet. The player's company enters the world without disturbing this balance (G2).
...
| D49 | Owner's fifth playtest, "there is no deal under deals but I am selling crude, where is it going?": Activity names the other side of every trade — the company, its region, and whether a deal was behind it — and says in so many words that a sale with no deal against it went on the open market, where the company posts its crude daily and the highest bid takes it. Rival names, regions and clearing prices are already public (G5), so nothing here is a leak |
...
- **There is a way out, at a price** (the owner's addition, 2026-09-22). A player can get out from
  ...
  a free pass — it costs a *multiple* of what the corner saved rather than a margin over it, since
  someone cutting corners in order to clean up later keeps the saving every time and pays only when
  a hint reaches them (which needs `E > saving ÷ hint rate`, so more than 1.7× at the rate this deck
  aims for: 1.2× before any hint, 2.5× at the first, 5× and counsel only once a file is opened);
...
**Deferred by the owner (D58, 2026-09-25).** Full mobile would reach into core elements rather than
restyling them, and the design is still moving while stage 6 balances — so it waits, in the place
§G10 always had it: the fifth and last delivery stage, a new interface rather than a repackaging.
This section stays as the brief for when it is picked up; the measurements above are what it starts
from, and the open questions below are what it has to answer.
...
- Localization
- Release stages 3–5: web portals, Steam and the mobile rebuild (§14.8, §12B, D58)
...
6. **No single closure strands a region:** for each of the ten chokepoints, closing it alone leaves every region at least one route to market.
...
| 5.2 | 2026-09-25 | Full mobile deferred by the owner (D58): a portrait interface reaches into core elements rather than restyling them, and the design is still moving while stage 6 balances. §12B stays as the brief for when it is picked up. Built instead, and verified at 375, 768, 1023, 1024 and 1440: a breakpoint under 1024 px that replaces the game screen with a notice saying what it needs, plus a line on the new-game page, which reads fine at 375 px, so nobody starts a game they cannot play. |
...
| 3.14 | 2026-09-21 | The Activity tab names who bought the crude and who it was bought from (D49). |
...
| 3.11 | 2026-09-21 | The owner's second playtest: a three-fifths/two-fifths screen, stock in barrels, the Activity tab in place of the Markets tab, the mission behind a button, and the clock saying why it stopped (D46). |
